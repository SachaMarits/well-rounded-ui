...
const Toast = (
  ...
  if (!toastsContainer)
    throw new Error(
      "You have to use <WellRoundedUI /> around your app to use Toast()"
    );
  ...
  const hide = () => root.unmount(toastsContainer);
  ...
  const dom = (
    <div className="toast-modal">
        ...
        { delay > 0 && <div className="toast-progress" style={{ backgroundColor: returnBackground(), animationDuration: `${delay}ms`, animationPlayState: "running" }} /> }
    </div>
  );
